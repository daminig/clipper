...
TEST_F(QueryFrontendTest, TestDecodeCorrectInputIntsBatch) {
  ...
  std::vector<folly::Try<Response>> responses =
      rh_.decode_and_handle_predict(std
                                    : move(d), "test", {}, "test_policy", 30000,
                                      InputType::Ints)
          .get();
...
TEST_F(QueryFrontendTest, TestDecodeCorrectUpdate) {
  ...
  FeedbackAck ack =
      rh_.decode_and_handle_update(std::move(d), "test", {}, "test_policy",
                                   InputType::Doubles)
          .get();
...
TEST_F(QueryFrontendTest, TestDecodeUpdateMissingField) {
  ...
  ASSERT_THROW(rh_.decode_and_handle_update(std::move(d), "test", {},
